...
5. 프롬프트 제어·위험·고위험 전문 조언만 고정 안전 답변으로 차단하고, 그 밖의 안전한 입력은 Gemini에 전달한다.
6. Gemini를 한 번 호출해 고양이 페르소나가 반영된 답변을 받는다.
7. 코딩과 일상 대화만 서버 범주 요약을 `CAT_MEMORIES`에 추가하고 일반 질문은 기억하지 않는다.
8. 답변과 기억 여부를 프런트에 반환한다.
...
- 일반 질문에는 자연스럽게 답하되 최신·실시간·불확실한 사실은 지어내지 말라는 규칙
