...
현재 진행 상태(2026-09-02): 공통 DB 기반 동기화와 **1. 공개 UUID DTO 변환**을 완료했다. 다음 작업은 **2. `balance_cost`와 `claim()` 계약 정리**다.
...
- [ ] 요청 payload를 정규 JSON으로 만드는 단일 함수를 구현한다.
- [ ] JSON 객체 키를 정렬한다.
- [ ] 불필요한 공백을 제거한다.
- [ ] UUID를 소문자 표준 문자열로 변환한다.
- [ ] `operation_type`을 포함한다.
- [ ] `request_id`를 해시 대상에서 제외한다.
- [ ] 가격, 잔액, 마일리지 등 서버 결정값을 제외한다.
- [ ] SHA-256 해시를 생성한다.
- [ ] 키 순서가 달라도 같은 해시가 생성되는지 테스트한다.
- [ ] 요청 내용이 다르면 다른 해시가 생성되는지 테스트한다.
