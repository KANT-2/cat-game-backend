...
현재 진행 상태(2026-09-04): 고양이 페르소나·기억 서비스와 단위 검증을 완료했다. `CATS.persona`는 삭제 대상이 아닌 고정 성격 데이터로 유지하며, 보유 고양이별 `CAT_MEMORIES`만 조회·추가·선택 삭제·전체 삭제한다. 다음 작업은 **11. FastAPI 라우터 연결**이다.
