...
자유 대화의 사용자 원문은 기억에 저장하지 않는다. 서버는 유니코드 정규화와 프롬프트 제어·위험·고위험
전문 조언 검사를 외부 AI 호출보다 먼저 수행한다. 이 세 종류만 고정된 안전 답변으로 차단하고, 코딩과
일상 대화 및 그 밖의 안전한 일반 질문은 생성 경계로 전달한다. 일반 질문은 `GENERAL`로 분류하며 장기
기억을 만들지 않는다. `CODING` 또는 `COMPANION`만 서버가 만든 범주 요약을 기억한다. 외부 응답은 길이와
시스템 지시 노출 여부를 다시 검사하며 실패하면 고정된 고양이 응답으로 대체한다.
...
4. 프롬프트 제어·위험·고위험 전문 조언은 서버가 차단하고, 나머지 코딩·일상·일반 질문은 최근 대화와 함께 Gemini `gemini-3.6-flash`에 한 번만 보낸다.
5. 구조화 결과의 `reply`를 반환한다. `CODING`과 `COMPANION`만 서버 범주 요약을 새 `CAT_MEMORIES` 행으로 commit하며 `GENERAL` 질문은 기억하지 않는다.
...
- BRONZE 50%, SILVER 20%, GOLD 0% 확률은 Python과 SQL에 동일하게 적용한다.
- 객관식 보기 순서와 정답 위치는 표시 세션을 만들 때 한 번 섞어 저장하며 새로고침과 같은 세션 재시도에서 유지한다. `correct_option`은 공개 응답에서 제외한다.
- `POST /api/v1/game/themes`에서 `item_catalog_key: null`은 보유 배경을 삭제하지 않고 기본 배경으로 되돌린다.
- 고양이 자유 대화 요청은 현재 창의 최근 발화 최대 10개를 `recent_messages`로 전달할 수 있다. 이 값은 응답 문맥에만 사용하고 원문 기억으로 저장하지 않는다.
