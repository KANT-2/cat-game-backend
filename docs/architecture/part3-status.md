...
- 여덟 카탈로그 고양이의 이름과 페르소나는 프런트 캐릭터인 포근이·먹구름·모카·호박이·은별이·단추·콩이·도토리와 일치한다.
...
- PWA 호환 가챠 풀에서 빠져 있던 `호박이(tabby)`를 1% 후보로 추가했다. `먹구름(ink)` 5%와
  나머지 다섯 고양이 각 1%를 합쳐 고양이 당첨 확률은 10%다.
