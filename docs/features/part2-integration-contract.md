...
2. 숙련도가 낮은 취약 개념 우선
3. 최근 20개 문제 우선 제외
4. 같은 우선순위 안에서는 개념별로 한 문제씩 번갈아 선택하고 코드형·객관식을 가능한 범위에서 혼합
5. 각 개념 안에서는 난이도 `BRONZE → SILVER → GOLD` 우선
6. 후보가 목표 개수보다 적으면 이미 고른 문제는 유지한 채 최근 문제 제외/취약 개념 조건을 순차 완화하고, 서로 다른 핵심 문항이 충분한 경우 요청한 `limit`까지 채움
7. 같은 사용자·게임 날짜·학습 이력에서는 안정된 순서 유지
8. 사용자 또는 게임 날짜가 바뀌면 해시 기반 후보 순서를 바꿔 오늘의 추천을 교체
