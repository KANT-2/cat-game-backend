...
코드형 문제의 `hint_text`는 줄바꿈으로 구분한 3단계 힌트다. Python은 시작 접근·핵심 연산·출력 확인, SQL은 대상 파악·절 작성·결과 확인 순서로 제공한다. 각 줄은 독립적으로 이해할 수 있어야 하며 정답 전체를 그대로 제공하지 않는다. 기존 단일 줄 힌트도 API 계약상 계속 허용한다.
...
추천용 취약 개념은 대표 개념의 최근 완료 시도 최대 10개 정답률로 계산한다. 최소 3회 시도하고 이 정답률이 50 이하일 때만 취약 개념이다. 화면에 표시하는 누적 숙련도는 별도 기준으로, 현재 해금된 활성 문제 중 정답 처리한 서로 다른 문제의 비율이다. 날짜가 바뀌어도 초기화되지 않으며 사용자가 학습 기록 초기화를 실행한 경우에만 그 시각 이후 기록으로 다시 계산한다. 응답에는 개념별 총 풀이 횟수, 정답 문제 수와 전체 문제 수도 포함한다.
...
추천은 숙련도가 낮은 취약 개념과 쉬운 난이도를 우선하며 최근 문제 20개를 먼저 제외한다. 취약 개념 후보만으로 요청한 수를 채우지 못하면 이미 고른 문제를 유지하면서 전체 개념으로 넓히고, 마지막 fallback에서만 최근 문제 제외를 푼다. 서로 다른 핵심 문항이 충분하면 요청한 `limit`까지 반환한다. 한 개념의 비슷한 문제가 목록 앞부분을 독점하지 않도록 개념별 후보를 한 문제씩 번갈아 선택하고 코드형·객관식도 가능한 범위에서 섞는다. 같은 사용자·게임 날짜·학습 이력에서는 재현 가능한 순서를 반환하되 사용자와 날짜가 달라지면 암호학적 해시 기반 순서가 바뀐다.

한 논리 문제는 코드형 정답과 선택적인 객관식 prompt·보기를 함께 가진다. 문제 목록은 사용자·게임 날짜·문제 UUID를 해시한 안정적인 표시 제안을 내려 Python과 SQL 모두 BRONZE는 약 50%, SILVER는 약 20%, GOLD는 0%가 객관식 카드로 보이게 한다. 사용자가 카드를 열면 이 제안을 표시 세션에 고정하므로 목록의 유형과 실제 풀이 유형이 일치한다. 직접 표시 세션을 만들 때 제안이 없으면 같은 확률 정책으로 선택한다. 객관식 보기는 표시 세션마다 섞어 정답 위치가 A~D 중 한 번호에 고정되지 않게 하며, 활성 세션 안에서는 새로고침해도 유형과 보기 순서를 유지한다.

같은 핵심 문항의 이야기 변형은 한 추천 페이지에 한 번만 포함한다. 사용자 티어는 Python과 SQL을 별도로 관리하며 BRONZE부터 시작한다. BRONZE 고유 문제 40개와 해당 난이도 각 개념의 50% 이상을 완료하면 SILVER, SILVER 고유 문제 45개와 각 개념의 60% 이상을 완료하면 GOLD를 해금한다. 승급 후 강등은 없고 학습 기록 초기화 시 두 과목 모두 BRONZE로 돌아간다. 잠긴 난이도는 문제 목록·추천·표시 세션·직접 학습 제출에서 제외한다.
