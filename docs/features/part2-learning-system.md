...
한 논리 문제는 코드형 정답과 선택적인 객관식 prompt·보기를 함께 가진다. 표시 세션을 새로 만들 때 Python과 SQL 모두 BRONZE는 50%, SILVER는 20%, GOLD는 0% 확률로 객관식을 선택하며, 나머지는 직접 작성형으로 제시한다. 따라서 BRONZE·SILVER 문제는 같은 논리 문제를 두 형식으로 경험할 수 있고 GOLD는 직접 작성형만 제공한다. 객관식 보기는 표시 세션마다 섞어 정답 위치가 A~D 중 한 번호에 고정되지 않게 하며, 활성 세션 안에서는 새로고침해도 유형과 보기 순서를 유지한다.
