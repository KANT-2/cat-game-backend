...
추천은 숙련도가 낮은 취약 개념과 쉬운 난이도를 우선하며 최근 문제 20개를 먼저 제외한다. 후보가 없으면 전체 개념으로 넓히고, 마지막 fallback에서만 최근 문제 제외를 푼다. 한 개념의 비슷한 문제가 목록 앞부분을 독점하지 않도록 개념별 후보를 한 문제씩 번갈아 선택하고 코드형·객관식도 가능한 범위에서 섞는다. 같은 사용자·게임 날짜·학습 이력에서는 재현 가능한 순서를 반환하되 사용자와 날짜가 달라지면 암호학적 해시 기반 순서가 바뀐다.
