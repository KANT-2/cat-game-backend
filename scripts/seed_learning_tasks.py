...
BRONZE = [
    Spec("variables", "두 수의 합", "두 정수 a, b를 읽고 합을 출력하세요.", "+ 연산자를 사용하세요.", "sum"),
    ...
    Spec("lists", "리스트 최댓값", "공백으로 구분된 정수 목록의 최댓값을 출력하세요.", "max를 사용할 수 있습니다.", "max"),
    Spec("tuples", "좌표 거리", "x와 y를 읽어 원점과의 맨해튼 거리를 출력하세요.", "좌표를 튜플로 묶어도 됩니다.", "manhattan"),
    Spec("sets", "중복 제거 개수", "공백으로 구분된 값에서 서로 다른 값의 개수를 출력하세요.", "set을 사용하세요.", "unique"),
    Spec("dictionaries", "단어 빈도", "공백으로 구분된 단어 중 첫 단어가 나온 횟수를 출력하세요.", "dict로 빈도를 세어보세요.", "frequency"),
    Spec("type_conversion", "섭씨 정수 변환", "실수 문자열 하나를 읽고 소수점을 버린 정수를 출력하세요.", "float 다음 int를 적용하세요.", "truncate"),
    Spec("boolean_logic", "범위 안인지 확인", "정수 n을 읽고 1 이상 100 이하면 YES를 출력하세요.", "비교 연산을 연결할 수 있습니다.", "range_check"),
]

SILVER = [
    Spec("list_comprehensions", "짝수 제곱 합", "정수 목록에서 짝수의 제곱 합을 출력하세요.", "필터와 변환을 함께 적용하세요.", "even_square_sum"),
    Spec("string_methods", "정규화한 단어 수", "문장을 소문자로 바꾼 뒤 공백 기준 단어 수를 출력하세요.", "lower와 split을 조합하세요.", "word_count"),
    Spec("sorting", "두 번째로 큰 수", "중복을 제거한 정수 목록에서 두 번째로 큰 수를 출력하세요.", "set과 sorted를 조합하세요.", "second_largest"),
    Spec("dictionaries", "최빈 문자", "소문자 단어에서 가장 자주 나온 문자를 출력하세요. 동률이면 알파벳순입니다.", "빈도와 정렬 기준을 함께 생각하세요.", "mode_char"),
    Spec("nested_loops", "약수 개수", "양의 정수 N의 약수 개수를 출력하세요.", "1부터 N까지 나누어 보세요.", "divisor_count"),
    ...
    Spec("iteration", "연속 증가 길이", "정수 목록에서 처음부터 연속으로 증가하는 구간 길이를 출력하세요.", "이전 값과 비교하세요.", "increasing_prefix"),
    Spec("parsing", "키-값 합계", "공백으로 구분된 key:value 항목들의 value 합을 출력하세요.", "':'로 한 번 분리하세요.", "kv_sum"),
]

GOLD = [
    Spec("functions", "회문 함수", "is_palindrome(text) 함수를 작성하고 입력 문자열의 결과를 YES/NO로 출력하세요.", "비교용 문자열을 뒤집어 보세요.", "palindrome"),
    Spec("algorithms", "괄호 균형", "is_balanced(text) 함수를 작성해 괄호가 올바르면 YES를 출력하세요.", "스택을 사용하세요.", "balanced"),
    Spec("dynamic_programming", "계단 경우의 수", "ways(n) 함수를 작성해 1칸 또는 2칸씩 N칸을 오르는 경우의 수를 출력하세요.", "이전 두 값을 저장하세요.", "stairs"),
    Spec("search", "이진 탐색 위치", "search(values, target) 함수로 정렬 목록의 target 인덱스를 출력하세요. 없으면 -1입니다.", "탐색 범위를 절반씩 줄이세요.", "binary_search"),
    Spec("data_modeling", "학생별 최고점", "name:score 목록에서 학생별 최고점 합계를 출력하세요.", "이름별 최대값을 dict에 저장하세요.", "best_scores"),
    Spec("recursion", "최대공약수", "gcd(a, b) 함수를 작성해 최대공약수를 출력하세요.", "유클리드 호제법을 사용하세요.", "gcd"),
    Spec("two_pointers", "목표 합 쌍", "정수 목록과 target을 읽고 합이 target인 서로 다른 인덱스 쌍의 수를 출력하세요.", "정렬 또는 해시 집합을 고려하세요.", "pair_sum"),
    Spec("aggregation", "구간 합 질의", "정수 목록과 l, r을 읽고 l부터 r까지(0-based, 양 끝 포함) 합을 출력하세요.", "누적 합 함수를 작성하세요.", "range_total"),
    Spec("graph_basics", "연결 요소 크기", "간선 목록과 시작점을 읽고 시작점에서 도달 가능한 정점 수를 출력하세요.", "인접 목록과 DFS/BFS를 사용하세요.", "reachable"),
    Spec("state_machines", "연속 문자 압축", "encode(text) 함수를 작성해 aaabb를 a3b2처럼 압축해 출력하세요.", "현재 문자와 개수를 유지하세요.", "run_length"),
]
    ...
    for difficulty, specs in (("BRONZE", BRONZE), ("SILVER", SILVER), ("GOLD", GOLD)):
        for variant in range(1, 6):
            for spec in specs:
                ...
                prompt = (
                    f"{variant_context} {spec.title}에 관한 설명 중 가장 올바른 것을 고르세요."
                    if is_choice else
                    f"{variant_context} {spec.prompt} 입력 형식과 출력 형식을 정확히 지키세요."
                )
                rows.append({
                    ...
                    "template_code": "" if is_choice else "# 여기에 풀이를 작성하세요.\n",
...
def main() -> None:
    ...
    try:
        ...
        if args.dry_run:
            ...
            db.commit()
        print({"benchmark_tasks_removed": removed, "created": created, "updated": updated, "dry_run": args.dry_run})
    finally:
        db.close()
